...
unsigned int nRetarget = 6; // 6 Blocks
unsigned int nStakeMinAge = 1 * 60 * 60 * 24; // 24 Hours
unsigned int nStakeMaxAge = -1;           // Unlimited
unsigned int nModifierInterval = 10 * 60; // Time to elapse before new modifier is computed
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // 6 blocks
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing; // 6 Blocks

//diff retarget -7 days
static const int64_t nTargetTimespan = 16 * 60;

int64_t devCoin = 0 * COIN;
int nCoinbaseMaturity = 7; //Amount of blocks for coins to mature
...
const string strMessageMagic = "REITIS Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // REITIS: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // REITIS: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// REITIS: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // REITIS: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // REITIS: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // REITIS: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // REITIS: track money supply and mint amount info
...
// REITIS: total coin age spent in transaction, in the unit of coin-days.
...
// REITIS: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // REITIS: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // REITIS: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // REITIS: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // REITIS: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // REITIS: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // REITIS: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // REITIS: getblocks may not obtain the ancestor block rejected
    ...
    // REITIS: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "REITIS", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "xRei-REITIS is created";
        CTransaction txNew;
        txNew.nTime = 1576756402;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1576756402;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 530706;
        if(fTestNet)
        {   
            block.nNonce   = 1882225;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xcda613ae54612cf8ef226450be772f925a1a81e184e5f0c285fb3e55a13f0317"));
        block.print();
        ...
        // REITIS: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x71, 0x3c, 0x23, 0x0c };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // REITIS: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // REITIS: tell downloading node about the latest block if it's
